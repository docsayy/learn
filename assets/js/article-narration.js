...
(() => {
  ...
  function ensureStylesheet() {
    ...
    try {
      const scriptURL = new URL(narrationScript.src);
      const stylesheetURL = new URL("../css/article-narration.css", scriptURL);
      if (scriptURL.search) stylesheetURL.search = scriptURL.search;
  ...
  function createPlayer() {
    ...
    floatingRoot.className = "narration-floating-root";
    floatingRoot.setAttribute("data-narration-ignore", "true");
    ...
    floatingRoot.innerHTML = `
      ...
      >
        ...
        <span class="narration-launcher-icon" aria-hidden="true">🔊</span>
        <span class="narration-launcher-state" aria-hidden="true"></span>
      </button>
      ...
      >
        <div class="narration-heading-row">
          ...
          <button class="narration-close" type="button" title="Close narration player" aria-label="Close narration player">×</button>
        </div>
        ...
        <div class="narration-controls">
          ...
          <button class="narration-button narration-reset" type="button" title="Start again from the beginning" aria-label="Reset narration to the beginning">
            <span aria-hidden="true">↺</span><span class="narration-button-label">Reset</span>
          </button>
        </div>
  ...
  function cacheControls() {
    const floatingRoot = state.player;
    state.launcher = floatingRoot.querySelector(".narration-launcher");
    state.controls = {
      panel: floatingRoot.querySelector(".narration-player"),
      launcher: state.launcher,
      launcherRing: floatingRoot.querySelector(".narration-launcher-ring"),
      launcherIcon: floatingRoot.querySelector(".narration-launcher-icon"),
      launcherState: floatingRoot.querySelector(".narration-launcher-state"),
      close: floatingRoot.querySelector(".narration-close"),
      status: floatingRoot.querySelector(".narration-status"),
      progressTrack: floatingRoot.querySelector(".narration-progress-track"),
      progressFill: floatingRoot.querySelector(".narration-progress-fill"),
      progressText: floatingRoot.querySelector(".narration-progress-text"),
      previous: floatingRoot.querySelector(".narration-skip-back"),
      play: floatingRoot.querySelector(".narration-play"),
      playIcon: floatingRoot.querySelector(".narration-play-icon"),
      playLabel: floatingRoot.querySelector(".narration-play-label"),
      next: floatingRoot.querySelector(".narration-skip-forward"),
      stop: floatingRoot.querySelector(".narration-stop"),
      reset: floatingRoot.querySelector(".narration-reset"),
      rate: floatingRoot.querySelector(".narration-rate"),
      voice: floatingRoot.querySelector(".narration-voice"),
      currentText: floatingRoot.querySelector(".narration-current-text")
    };
  }
  ...
  function bindEvents() {
    ...
    controls.launcher.addEventListener("click", () => {
    ...
    controls.reset.addEventListener("click", resetNarration);
  ...
  function setPanelOpen(open) {
    ...
    state.controls.panel.hidden = !state.panelOpen;
    state.controls.launcher.setAttribute("aria-expanded", String(state.panelOpen));
    state.controls.launcher.setAttribute(
      "aria-label",
      state.panelOpen
        ? "Close article narration player"
        : "Open article narration player"
    );
    state.player.classList.toggle("narration-panel-open", state.panelOpen);

    if (state.panelOpen) {
      window.setTimeout(() => state.controls.play.focus({ preventScroll: true }), 0);
    }
  }
  ...
  function resetNarration() {
    ...
    updateInterface("Reset to the beginning");
    saveState(true);
  }
  ...
  function highlightCurrentChunk(shouldScroll) {
    ...
    if (shouldScroll) {
      ...
      const topBuffer = 110;
      const floatingPanelHeight =
        isMobileView() && state.panelOpen
          ? Math.ceil(state.controls.panel?.getBoundingClientRect().height || 0)
          : 0;
      const bottomBuffer = floatingPanelHeight ? floatingPanelHeight + 150 : 90;
      const isOutsideView =
  ...
  function updateInterface(statusMessage) {
    ...
    controls.launcher.style.setProperty("--narration-progress", `${percent * 3.6}deg`);

    const isPlaying = state.mode === "playing";
    controls.launcher.classList.toggle("is-playing", isPlaying);
    controls.launcher.classList.toggle("is-paused", state.mode === "paused");
    controls.launcherState.textContent = isPlaying ? "•" : "";
    ...
    controls.reset.disabled = !total || (state.currentIndex === 0 && state.mode === "idle");
