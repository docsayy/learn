/*
 * Learn Medicine — Article Tools v4.3.0
 * Browser-native narration using the Web Speech API.
 *
 * Combined narration module. This file is intended to be bundled with the
 * reading-progress module and saved as assets/js/article-progress.js.
 * Narration styles are included in assets/css/article.css.
 */
...
(() => {
  ...
  const state = {
    ...
    routeAudio: null,
    routeAudioUrl: "",
    routeAudioPrimed: false,
    routeAudioFailed: false
  };
  ...
  function isAndroidDevice() {
    ...
    return /Android/i.test(uaDataPlatform) || /Android/i.test(navigator.userAgent || "");
  }

  function createSilentWavUrl() {
    // A one-second, mono PCM WAV containing digital silence. Keeping a real
    // HTMLMediaElement playing helps Android treat Chrome as active media,
    // which can improve routing to Bluetooth and Android Auto.
  ...
  function ensureRouteAudio() {
    if (!isAndroidDevice()) return null;
    if (state.routeAudio) return state.routeAudio;

    const audio = document.createElement("audio");
    audio.setAttribute("aria-hidden", "true");
    audio.setAttribute("data-narration-ignore", "true");
    audio.preload = "auto";
    audio.loop = true;
    ...
    state.routeAudioUrl = createSilentWavUrl();
    audio.src = state.routeAudioUrl;
    document.body.appendChild(audio);
    state.routeAudio = audio;

    if ("mediaSession" in navigator) {
      try {
        navigator.mediaSession.metadata = new MediaMetadata({
          title: document.title || "Learn Medicine article",
          artist: "Learn Medicine",
          album: "Article narration"
        });
      } catch {
        // Metadata is optional and narration still works without it.
      }
    }
  ...
  async function primeAndroidMediaRoute() {
    const audio = ensureRouteAudio();
    if (!audio || state.routeAudioFailed) return false;
    if (!audio.paused) {
      state.routeAudioPrimed = true;
      return true;
    }
    ...
    try {
      ...
      state.routeAudioFailed = true;
      return false;
    }
  }

  function pauseAndroidMediaRoute() {
    if (!state.routeAudio || state.routeAudio.paused) return;
    state.routeAudio.pause();
    if ("mediaSession" in navigator) {
      try {
  ...
  function stopAndroidMediaRoute() {
    if (!state.routeAudio) return;
    state.routeAudio.pause();
    try {
      state.routeAudio.currentTime = 0;
    } catch {
      // Some browsers may reject seeking a generated audio source.
    }
    state.routeAudioPrimed = false;
    if ("mediaSession" in navigator) {
      try {
  ...
  function destroyAndroidMediaRoute() {
    stopAndroidMediaRoute();
    state.routeAudio?.remove();
    state.routeAudio = null;
    if (state.routeAudioUrl) URL.revokeObjectURL(state.routeAudioUrl);
    state.routeAudioUrl = "";
  }
...
/*
 * Learn Medicine — Reading Progress (combined module)
 *
 * This module runs from the same article-progress.js file as narration.
 * It updates an existing top progress bar when present and always maintains the
 * floating percentage badge used by the Learn Medicine article theme.
 */

(() => {
  "use strict";

  const STORAGE_PREFIX = "learnMedicineArticleProgress:v2:";
  const LEGACY_STORAGE_PREFIX = "learnMedicineArticleProgress:v1:";
  ...
  function createProgressBadge() {
    ...
    badge.className = "reading-progress";
    badge.setAttribute("data-reading-progress-auto", "true");
    badge.textContent = "0%";
    document.body.appendChild(badge);
    return badge;
  }

  function findTopProgressBar() {
    const candidate =
      document.getElementById("reading-progress") ||
      document.getElementById("progressBar") ||
      document.querySelector(".progress-bar");

    // A .reading-progress element is the floating badge, not the thin top bar.
    return candidate?.classList?.contains("reading-progress") ? null : candidate;
  }

  function findProgressBadge() {
    return document.querySelector(".reading-progress") || createProgressBadge();
  }
  ...
  function loadLegacyProgress() {
    try {
      const saved = JSON.parse(
        localStorage.getItem(`${LEGACY_STORAGE_PREFIX}${location.pathname}`) || "null"
      );
      return Number.isFinite(Number(saved?.percent)) ? Number(saved.percent) : null;
    } catch {
      return null;
    }
  }
  ...
  function saveProgress(percent) {
    ...
    try {
      ...
      // Reading progress remains functional when storage is unavailable.
    }
  }
  ...
  function renderProgress() {
    ...
    if (progressBar) {
      ...
      progressBar.dataset.progress = String(roundedPercent);
      progressBar.setAttribute("aria-valuenow", String(roundedPercent));
    }

    if (progressBadge) {
      progressBadge.textContent = `${roundedPercent}%`;
      progressBadge.dataset.progress = String(roundedPercent);
      progressBadge.setAttribute("aria-valuenow", String(roundedPercent));
      progressBadge.setAttribute(
        "aria-label",
        `Article reading progress: ${roundedPercent} percent`
      );
    }

    document.documentElement.dataset.readingProgress = String(roundedPercent);
  ...
  function initialize() {
    progressBar = findTopProgressBar();
    progressBadge = findProgressBadge();
    ...
    progressBadge.setAttribute("role", "progressbar");
    progressBadge.setAttribute("aria-valuemin", "0");
    progressBadge.setAttribute("aria-valuemax", "100");

    // Read legacy data once so an older saved record remains discoverable by the
    // site. We do not auto-scroll, because opening an article should never jump
    // unexpectedly without a direct user action.
    loadLegacyProgress();
